...
from rss_visualization import RssUnstructuredSceneVisualizer, RssBoundingBoxVisualizer, \
    RssStateVisualizer  # pylint: disable=relative-import
...
class World(object):
    ...
    def restart(self):
        ...
        self.rss_sensor = RssSensor(self.player, self.world,
                                    self.rss_unstructured_scene_visualizer, self.rss_bounding_box_visualizer,
...
class Camera(object):
    ...
    def __init__(self, parent_actor, display_dimensions):
        ...
        self.sensor = self._parent.get_world().spawn_actor(bp, carla.Transform(carla.Location(
            x=-5.5, z=2.5), carla.Rotation(pitch=8.0)), attach_to=self._parent,
...
class VehicleControl(object):
    ...
    def render(self, display):
        if self._mouse_steering_center:
            display.blit(
                self._surface, (self._mouse_steering_center[0] - self.MOUSE_STEERING_RANGE,
                ...
                if rss_proper_response:
                    if not (pygame.key.get_mods() & KMOD_CTRL):
                        vehicle_control = self._restrictor.restrict_vehicle_control(
                            vehicle_control, rss_proper_response, self._world.rss_sensor.ego_dynamics_on_route,
