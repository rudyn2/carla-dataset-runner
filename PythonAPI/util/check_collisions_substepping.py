...
class CollisionScenarioTester():
    ...
    def test_scenario(self, fps=20, fps_phys=100, repetitions=1, sim_tics=100):
        output_str = "Testing Determinism in %s for %3d render FPS and %3d physics FPS -> " % (
        self.scenario_name, fps, fps_phys)
        ...
        if determ_repet[0] != repetitions:
            print("Error!!! Scenario %s is not deterministic: %d / %d" % (
            self.scenario_name, determ_repet[0], repetitions))
